...
The PC app normally sets up the tunnel ([§ 10.1](../architecture/10-connection-levels.md)). When testing by hand:
...
- Format with `ktlint` ([§ 20](../rules/20-coding-rules.md), rule 8).
- APK size stays within budget — ≤ 6 MB per ABI on a release build ([§ 15](../rules/15-performance-budgets.md)).
