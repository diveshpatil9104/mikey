...
## 10.1 Level 1 — USB debugging (ADB)
...
**PC side (`adb.rs`):**
- Uses the `adb` found on `PATH` if present (avoids killing a developer's own ADB server with a version mismatch); otherwise a bundled `adb` from Android platform-tools shipped next to `mikeyd` (verify redistribution terms before release — scrcpy ships the same way).
...
**Phone side:** `AdbTransport.probe()` = try TCP connect to `127.0.0.1:7653` with a 300 ms timeout. Succeeds only if a reverse tunnel exists. Cheap enough to run on every trigger ([§ 10.5](10-connection-levels.md)).
...
- Send a UDP discovery probe to that interface's broadcast address ([§ 10.7](10-connection-levels.md)). The PC answers with its IP and port.
...
## 10.5 When the phone probes (event-driven, battery-friendly)
...
## 10.6 Upgrade & downgrade (make-before-break)
...
## 10.7 Discovery beacon (UDP 7654)
