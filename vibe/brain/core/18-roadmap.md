...
Each phase ends with a real-device test on at least two phones (one Pixel/near-stock, one Samsung or Xiaomi) and one Windows + one Linux machine, and with the budgets in [§ 15](../rules/15-performance-budgets.md) checked.
...
Android
- [ ] Full UI per [§ 6](06-phone-ux.md) (split halves, chevron + drawer, status dot 3 colors, rotation behavior)
- [ ] Settings persistence per [§ 6.6](06-phone-ux.md)
- [ ] `TransportManager` with all four transports, event-driven probing ([§ 10.5](../architecture/10-connection-levels.md)), make-before-break handover ([§ 10.6](../architecture/10-connection-levels.md))
...
PC
- [ ] Tray icon + menu per [§ 7.2](07-pc-ux.md); autostart at login
- [ ] `beacon.rs`, `bt.rs` (Windows + Linux)
- [ ] `session.rs`: tokens, trust rules ([§ 11.3](../architecture/11-sessions-trust.md)), ask-before-join prompts, one-active rule, handover with 30 s session hold
...
- [ ] Refactor pass: modules as in [§ 17](../architecture/17-repo-structure.md) (Phase 1 was allowed to be rough)
...
**Done when:** v1.0.0 is tagged with signed APK + Windows installer + Linux packages attached, README complete, and the test matrix ([§ 19](../rules/19-test-matrix.md)) passes.

- [ ] Optional: AES-GCM on Level 4; UDP audio path if measurements justify it ([§ 12.6](../architecture/12-wire-protocol.md))
