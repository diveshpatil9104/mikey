...
3. **Lightweight by construction.** Small binaries, few dependencies, near-zero idle CPU, no background work when nothing is streaming. Every dependency must justify its weight (see [§ 15](../rules/15-performance-budgets.md)).
...
6. **Private by default.** Mic and camera start **off**. A device you haven't seen before cannot silently inject audio into your PC over Wi‑Fi (see [§ 11](../architecture/11-sessions-trust.md)).
