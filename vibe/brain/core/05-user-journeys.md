...
## 5.1 First-time setup (PC)

1. Download the installer from GitHub Releases, run it.
2. The installer: copies `mikeyd`, registers the virtual camera (one admin prompt on Windows), adds a firewall rule for private networks, enables autostart, launches the tray app.
...
## 5.3 The first connection on each level (one-time friction)
...
1. Open Mikey on the phone (or plug in via USB with debugging on — the PC can open the app for you, [§ 7.3](07-pc-ux.md)).
